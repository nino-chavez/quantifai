...
Genuinely needed for a Commerce.com internal deployment: the ingest chain, org/invite model, encrypted credentials, cron aggregation. Speculative: everything above, plus multi-tenant readiness (one seeded org, `20260324000002_seed.sql`).
...
- **Stage 1 research would NOT have killed it in March 2026** — and that matters for the gate's credibility. At build time, Copilot's API was in preview, Cursor had no spend controls, Anthropic had no per-user cost console, and Larridin/Torii's products didn't exist. Honest research would have found genuine whitespace *and* forced the three-buyer fragmentation into the open, most plausibly reshaping the initiative into "internal Commerce.com deployment with a named stakeholder first, commercial later." The sibling-project scan would also have surfaced rally-hq's auth as the canonical pattern on day one instead of commit 20.
...
- **Kill-gate:** the pilot profile requires a named Commerce.com stakeholder (or equivalent real buyer) whose walkthrough is the citation. No stakeholder → the initiative ends at Stage 1 as a documented "retire," at the cost of days.
